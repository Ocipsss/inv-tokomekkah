...
import { 
  ...
  MapPin, RefreshCw, ShieldCheck, User 
} from "lucide-react";
...
export default function Sidebar() {
  ...
  useEffect(() => {
    ...
    setRole(userRole as string || 'staff');
  }, [pathname]);
  ...
  return (
    <>
      ...
      {/* 2. OVERLAY */}
      {isOpen && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[110] md:hidden transition-opacity" onClick={() => setIsOpen(false)} />
      )}
      ...
      `}>
        ...
        {/* Menu Items */}
        <nav className="flex-1 overflow-y-auto px-4 custom-scrollbar mt-6 space-y-8">
        ...
        <div className="p-4 mt-auto space-y-2">
          <button 
            ...
            className="flex items-center gap-3 w-full px-4 py-3 text-slate-400 hover:text-blue-600 transition-colors text-[10px] font-bold uppercase tracking-widest group"
          >
          ...
          <button 
            ...
            className="flex items-center justify-center gap-3 w-full p-4 text-white bg-red-500 hover:bg-red-600 rounded-2xl transition-all shadow-lg shadow-red-100 active:scale-95"
          >
            <LogOut size={18} />
