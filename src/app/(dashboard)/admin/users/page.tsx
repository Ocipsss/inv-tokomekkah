...
import { 
  UserPlus, ShieldCheck, Trash2, 
  Mail, Loader2, X, CheckCircle2,
  Fingerprint
} from "lucide-react";
    ...
    <div className="p-2 pb-24 max-w-full mx-auto">
      ...
      <div className="flex justify-between items-center mb-6 px-2">
        ...
        <button 
          ...
          className="bg-blue-600 text-white p-2.5 rounded-xl shadow-md active:scale-95 transition-all"
        >
          <UserPlus size={18} />
        </button>
      </div>
          ...
          <div key={staff.id} className="bg-white p-3 rounded-xl border border-slate-100 shadow-sm">
            ...
            <div className="grid grid-cols-2 gap-2 border-t border-slate-50 pt-3">
              <button 
                ...
                className="flex items-center justify-center gap-2 p-2 rounded-lg bg-slate-50 text-slate-600 text-[10px] font-bold uppercase"
              >
                <ShieldCheck size={14} />
                Role
              </button>
              <button 
                ...
                className="flex items-center justify-center gap-2 p-2 rounded-lg bg-red-50 text-red-500 text-[10px] font-bold uppercase"
              >
                <Trash2 size={14} />
                Hapus
              </button>
      ...
      {/* MODAL ADD STAFF - Versi Compact */}
      {isModalOpen && (
        <div className="fixed inset-0 z-[100] flex items-end sm:items-center justify-center p-4">
          <div className="absolute inset-0 bg-slate-900/60 backdrop-blur-sm" onClick={() => !isSaving && setIsModalOpen(false)} />
          
          <form onSubmit={handleAddStaff} className="relative bg-white w-full max-w-sm rounded-2xl shadow-2xl overflow-hidden animate-in slide-in-from-bottom-10 duration-200 p-6">
            ...
            <div className="space-y-3">
              ...
              />
              <select 
                className="w-full p-3 bg-slate-50 border-none rounded-xl outline-none focus:ring-2 focus:ring-blue-500 text-xs font-black uppercase"
                value={newStaff.role}
            ...
            <button 
              disabled={isSaving}
              className="w-full mt-5 bg-blue-600 text-white p-3.5 rounded-xl font-black text-xs uppercase tracking-widest active:scale-95 transition-all flex items-center justify-center gap-2"
            >
