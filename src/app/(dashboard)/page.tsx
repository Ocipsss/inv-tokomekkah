    ...
    <div className="p-3 pb-24 max-w-2xl mx-auto space-y-6">
      ...
      <section className="space-y-3">
        ...
        {lowStockItems.length > 0 ? (
          <div 
            ...
            className="bg-red-50 border border-red-100 rounded-xl p-3 flex items-center gap-3 cursor-pointer active:scale-95 transition-all"
          >
            ...
            <div className="flex-1">
              ...
              <p className="text-[9px] text-red-600 mt-0.5 uppercase font-bold">
                Ada {lowStockItems.length} item hampir habis di gudang.
              </p>
            </div>
      ...
      <section className="space-y-3">
        <div className="flex items-center justify-between px-1">
          ...
          <button onClick={() => router.push('/products')} className="text-[9px] font-black text-blue-600 uppercase">Lihat Semua</button>
        </div>
          ...
          {recentUpdates.length > 0 ? recentUpdates.map((item) => (
            <div key={item.id} className="flex items-center justify-between p-3 bg-white border border-slate-50 rounded-xl hover:border-blue-100 transition-all">
              <div className="flex items-center gap-3 min-w-0">
                <div className="bg-slate-50 text-slate-400 p-2 rounded-lg group-hover:bg-blue-50 transition-colors">
      ...
      <button className="w-full bg-slate-900 text-white p-4 rounded-xl flex items-center justify-between group relative overflow-hidden active:scale-[0.98] transition-all shadow-xl shadow-slate-200">
        <div className="relative z-10 flex items-center gap-3 text-left">
          ...
          <div>
            ...
            <p className="text-[8px] opacity-50 uppercase font-bold">Analisis Alur Keluar Masuk Barang</p>
          </div>
        </div>
        <ArrowUpRight className="relative z-10 opacity-50 group-hover:translate-x-1 group-hover:-translate-y-1 transition-transform" size={20} />
      ...
      <div className="pt-6 pb-2 text-center">
        <span className="text-[8px] font-black text-slate-300 uppercase tracking-[0.4em]">
          TokoMekkah V1.0 Stable Build
        </span>
      </div>
    </div>
  );
