...
export default function PublishersPage() {
  ...
  const deletePublisher = async (id: number) => {
    if (confirm("⚠️ Hapus penerbit ini? Data barang dengan penerbit ini tidak akan terhapus, tapi kategori akan hilang dari pilihan.")) {
      ...
      <form onSubmit={addPublisher} className="flex gap-2">
        <div className="relative flex-1">
          ...
          <input 
            ...
            className="w-full p-4 pl-11 bg-white border border-slate-100 rounded-xl outline-none focus:ring-2 focus:ring-orange-500 text-[11px] font-black uppercase shadow-sm transition-all"
            value={newPub}
        ...
        {publishers ? (
          <div className="grid grid-cols-1 gap-2">
            {publishers.map((pub) => (
              <div 
                key={pub.id} 
                className="bg-white p-3 rounded-xl border border-slate-50 shadow-sm flex items-center justify-between group hover:border-orange-200 transition-all"
              >
                <div className="flex items-center gap-3">
                  <div className="w-9 h-9 bg-orange-50 rounded-lg flex items-center justify-center text-orange-600 shrink-0">
                    <BookOpen size={16} />
                  </div>
