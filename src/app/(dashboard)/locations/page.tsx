...
export default function LocationsPage() {
  ...
  return (
    <div className="p-2 pb-24 max-w-full mx-auto space-y-4">
      ...
      {/* Grid List - Otomatis bagi kolom di layar lebar */}
          ...
          filteredData?.map((item: any) => (
            <div key={item.id} className="flex items-center justify-between p-3 bg-white rounded-xl border border-slate-100 group hover:border-blue-200 transition-all shadow-sm">
              <div className="flex items-center gap-3">
                <div className="w-8 h-8 bg-slate-50 rounded-lg flex items-center justify-center text-slate-400 font-black text-[10px] border border-slate-100">
                  #
                </div>
              ...
              <button 
                ...
                className="text-slate-200 hover:text-red-500 p-1.5 hover:bg-red-50 rounded-lg transition-all"
              >
                <Trash2 size={16} />
              </button>
            </div>
